...
| `--cache-dir` | Where controller snapshots are read/written. A snapshot is a full inventory of your network, so keeping it outside a git repository is worth doing: set $UNIFI_MAP_CACHE_DIR once instead of passing this every time (default: cache) | `cache` |
